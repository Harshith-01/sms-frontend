...
export default function MyDocuments() {
  ...
  return (
    <div className="documents-page">
      <Card title={<div className="page-header"><h2>My Documents ({documents.length})</h2><Button type="primary" icon={<UploadOutlined />} onClick={() => navigate('/teacher/documents/upload')}>Upload</Button></div>}>
        <Table columns={columns} dataSource={documents} loading={loading} rowKey="id" pagination={{ pageSize: 10 }} />
      </Card>
      <Modal title="Preview" open={previewVisible} onCancel={() => setPreviewVisible(false)} footer={[<Button key="close" onClick={() => setPreviewVisible(false)}>Close</Button>]} width={800}>
        {selectedDoc && (
          <div className="document-preview">
            ...
            <div className="doc-preview-container">
              {selectedDoc.file_url?.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? <Image src={selectedDoc.file_url} alt="Document" style={{ width: '100%' }} /> : <iframe src={selectedDoc.file_url} style={{ width: '100%', height: '500px', border: 'none' }} />}
            </div>
          </div>
        )}
      </Modal>
